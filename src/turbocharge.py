...
def is_password_valid(password: str):
    proc = subprocess.Popen('sudo -k -S -l'.split(), stdin=subprocess.PIPE,
...
class Debugger:
    def debug(self, password: str, error: bytes):
        ...
        if 'sudo: 1 incorrect password attempt' in error:
            ...
            click.echo(click.style(
                f'Cause: Wrong Password Entered. Code: 001', fg='yellow', bold=True, blink=True))
            return
        elif platform == 'darwin':
            click.echo(click.style(
                '✅ Successful Debugging! ✅ \n', fg='green', bold=True))
            click.echo(click.style(
                f'Cause: Incompatible Platform. Turbocharge doesn\'t support macOS yet. Code: 005', fg='yellow', bold=True, blink=True))
            return
        elif platform == 'win32':
            click.echo(click.style(
                '✅ Successful Debugging! ✅ \n', fg='green', bold=True))
            click.echo(click.style(
                f'Cause: Incompatible Platform. Turbocharge doesn\'t support Windows 10/8/7/XP yet. Code: 010', fg='yellow', bold=True, blink=True))
            return
        else:
    ...
    def install_task(self, package_name: str, script: str, password: str, test_script: str, tests_passed):
        try:
            ...
            if proc.returncode != 0:
                click.echo(click.style('❎ Installation Failed... ❎',
            ...
            for _ in range(21, 60):
                time.sleep(0.002)
                testing_bar.next()
            proc = Popen(test_script.split(), stdin=PIPE,
...
class Uninstaller:
    ...
    def clean(self, password: str):
        try:
            ...
            click.echo('\n')
            click.echo(click.style(
                '🎉 Successfully Cleaned Turbocharge! 🎉', fg='green'))
        except subprocess.CalledProcessError as e:
            click.echo(e.output)
            click.echo('An Error Occured During Installation...', err=True)
        ...
        if platform == 'linux':
            ...
            if package_name in devpackages:
                ...
                turbocharge.install_task(
                    devpackages[package_name], f'sudo -S apt-get install -y {package_name}', password, f'{package_name} --version', [f'{devpackages[package_name]} Version'])

            if package_name in applications:
                show_progress(finding_bar)
                turbocharge.install_task(
                    applications[package_name], f'sudo -S snap install --classic {package_name}', password, '', [])
            ...
            if package_name == 'chrome':
                ...
                try:
                    ...
                    proc.wait()
                    second = Popen("sudo -S apt-get install -y ./google-chrome-stable_current_amd64.deb".split(
                    ), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    # Popen only accepts byte-arrays so you must encode the string
                    ...
                    click.echo(e.output)
                    click.echo(
                        'An Error Occured During Installation...', err=True)
            ...
            if package_name == 'anaconda':
                ...
                try:
                    ...
                    for _ in range(61, 91):
                        ...
                        installer_progress.next()
                    os.system(
                        f'echo "export PATH="/home/{username}/anaconda3/bin:$PATH"" >> ~/.bashrc')
                    # Popen only accepts byte-arrays so you must encode the string
                    proc.communicate(password.encode())
            ...
            if package_name == 'miniconda':
                ...
                try:
                    ...
                    for _ in range(1, 35):
                        ...
                        installer_progress.next()
                    os.system(
                        "wget https://repo.anaconda.com/archive/Anaconda3-2020.07-Linux-x86_64.sh -O ~/miniconda.sh")
                    for _ in range(35, 61):
                        time.sleep(0.01)
                        installer_progress.next()
                    os.system('bash ~/anaconda.sh -b -p $HOME/anaconda3')
    ...
    for package in packages:
        if package in devpackages:
            uninstaller.uninstall(
                f'sudo -S apt-get remove -y {package}', password, package_name=devpackages[package])

        if package in applications:
            uninstaller.uninstall(
                f'sudo snap remove {package}', password, package_name=applications[package])
        ...
        if package == 'anaconda':
            try:
                ...
                click.echo(e.output)
                click.echo(
                    'An Error Occured During Uninstallation...', err=True)
...
def hyperpack(hyperpack_list):
    ...
    if platform == 'linux':
        ...
        for hyperpack in hyperpacks:
            ...
            for package in packages:
                turbocharge.install_task(
                    devpackages[package], f'sudo -S apt-get install -y {package}', password, f'{package} --version', [f'{devpackages[package]} Version'])
            for app in apps:
                turbocharge.install_task(
                    applications[package], f'sudo -S snap install --classic {package}', password, '', [])
